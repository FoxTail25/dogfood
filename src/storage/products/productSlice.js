...
export const fetchChangrLikeProduct = createAsyncThunk(
    'product/fetchChangrLikeProduct',
    async function (
        product,
        { rejectWithValue, fulfillWithValue, dispatch, getState, extra: api }
    ) {
        try {
            ...
            const data = await api.changeLikeProd(product._id, liked);
...
const productsSlice = createSlice({
        ...
        sortedProducts: (state, action) => {
            switch (action.payload) {
                ...
                case SORTED.SALE:
                    ...
                    state.currentSort = action.payload;
                    break;
                default: state.data = state.data.sort((a, b) => b.discount - a.discount);
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchChangrLikeProduct.fulfilled, (state, action) => {
